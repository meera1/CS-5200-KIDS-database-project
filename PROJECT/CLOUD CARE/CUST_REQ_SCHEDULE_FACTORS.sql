...
EXECUTE dbms_output.put_line('CUST_REQ_SCHEDULE_FACTORS node started: ' || SYSTIMESTAMP);
EXECUTE dbms_output.put_line('');
...
BEGIN
  ...
  v_sql := 
    ...
    (
      select /*+ inline */ "CUST_REQ_SCHEDULE_FACTORS"."EXECUTION_TIME", 
"CUST_REQ_SCHEDULE_FACTORS"."CUST_ID", 
"CUST_REQ_SCHEDULE_FACTORS"."ID", 
"CUST_REQ_SCHEDULE_FACTORS"."RESOURCES_REQUIRED", 
"CUST_REQ_SCHEDULE_FACTORS"."WAIT_TIME" 
from &DATA_SOURCE_2 "CUST_REQ_SCHEDULE_FACTORS" 
    )';  
  execSQL(v_sql);
  
  recordOutput('10008', 'CUST_REQ_SCHEDULE_FACTORS', 'DataSourceNode', NULL, NULL, NULL, v_view_name, 'VIEW', NULL, 'Output Data');
EXCEPTION WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(-20999, formatErrorStack('CUST_REQ_SCHEDULE_FACTORS', SQLERRM, DBMS_UTILITY.FORMAT_ERROR_BACKTRACE()));
END;
/
EXECUTE dbms_output.put_line('');
EXECUTE dbms_output.put_line('CUST_REQ_SCHEDULE_FACTORS node completed: ' || SYSTIMESTAMP);
EXECUTE dbms_output.put_line('');

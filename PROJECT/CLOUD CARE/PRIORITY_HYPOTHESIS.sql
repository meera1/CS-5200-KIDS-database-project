...
EXECUTE dbms_output.put_line('PRIORITY_HYPOTHESIS node started: ' || SYSTIMESTAMP);
EXECUTE dbms_output.put_line('');
...
BEGIN
  ...
  v_sql := 'CREATE TABLE &CREATE_TABLE_3 (
    "CLAS_SVM_2_6_PRED"             NUMBER,
    "CLAS_SVM_2_6_PROB"             NUMBER,
    "CUST_ID"                       NUMBER,
  ...
  v_sql := 'INSERT  INTO &CREATE_TABLE_3
    SELECT /*+ inline  */ 
    "CLAS_SVM_2_6_PRED",
    "CLAS_SVM_2_6_PROB",
    "CUST_ID",
    ...
    "WAIT_TIME",
    "PRIORITY"
    FROM (SELECT * FROM ' ||getInputSource('10038')||')';
  execSQL(v_sql);
  
  recordOutput('10048', 'PRIORITY_HYPOTHESIS', 'CreateTableNode', NULL, NULL, NULL, v_table_name, 'TABLE', NULL, 'Output Data');
EXCEPTION WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(-20999, formatErrorStack('PRIORITY_HYPOTHESIS', SQLERRM, DBMS_UTILITY.FORMAT_ERROR_BACKTRACE()));
END;
/
EXECUTE dbms_output.put_line('');
EXECUTE dbms_output.put_line('PRIORITY_HYPOTHESIS node completed: ' || SYSTIMESTAMP);
EXECUTE dbms_output.put_line('');
